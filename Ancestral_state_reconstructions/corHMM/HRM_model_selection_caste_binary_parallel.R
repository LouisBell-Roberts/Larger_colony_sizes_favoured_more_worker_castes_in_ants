...
corHMM_multiphylo_1rat_ER <- function(multiphylo, trait_data) {
  ...
  foreach(i = 1:length(multiphylo)) %dopar% {
    ...
    saveRDS(output, file = paste0("corHMM_ASR_Caste_only_1rat_ER_", i, ".rda"))
    cat("Saved output to", paste0("corHMM_ASR_Caste_only_1rat_ER_", i, ".rda"), "\n")
  }
}
...
corHMM_multiphylo_1rat_ARD <- function(multiphylo, trait_data) {
  ...
  # Run corHMM ancestral state reconstruction for each phylogeny
  foreach(i = 1:length(multiphylo)) %dopar% {
    output <- corHMM(phy = multiphylo[[i]], data = trait_data, rate.cat = 1, model = "ARD")
    
    # Save output to a file
    saveRDS(output, file = paste0("corHMM_ASR_Caste_only_1rat_ARD_", i, ".rda"))
    cat("Saved output to", paste0("corHMM_ASR_Caste_only_1rat_ARD_", i, ".rda"), "\n")
  }
}
...
corHMM_multiphylo_2rat_ER <- function(multiphylo, trait_data) {
  ...
  foreach(i = 1:length(multiphylo)) %dopar% {
    ...
    # Save output to a file
    saveRDS(output, file = paste0("corHMM_ASR_Caste_only_2rat_ER_", i, ".rda"))
    cat("Saved output to", paste0("corHMM_ASR_Caste_only_2rat_ER_", i, ".rda"), "\n")
  }
}
...
corHMM_multiphylo_2rat_ARD <- function(multiphylo, trait_data) {
  ...
  # Run corHMM ancestral state reconstruction for each phylogeny
  foreach(i = 1:length(multiphylo)) %dopar% {
    output <- corHMM(phy = multiphylo[[i]], data = trait_data, rate.cat = 2, model = "ARD")
    
    # Save output to a file
    saveRDS(output, file = paste0("corHMM_ASR_Caste_only_2rat_ARD_", i, ".rda"))
    cat("Saved output to", paste0("corHMM_ASR_Caste_only_2rat_ARD_", i, ".rda"), "\n")
  }
}
...
corHMM_multiphylo_3rat_ER <- function(multiphylo, trait_data) {
  ...
  # Run corHMM ancestral state reconstruction for each phylogeny
  foreach(i = 1:length(multiphylo)) %dopar% {
    output <- corHMM(phy = multiphylo[[i]], data = trait_data, rate.cat = 3, model = "ER")
    
    # Save output to a file
    saveRDS(output, file = paste0("corHMM_ASR_Caste_only_3rat_ER_", i, ".rda"))
    cat("Saved output to", paste0("corHMM_ASR_Caste_only_3rat_ER_", i, ".rda"), "\n")
  }
}
...
corHMM_multiphylo_3rat_ARD <- function(multiphylo, trait_data) {
  ...
  # Run corHMM ancestral state reconstruction for each phylogeny
  foreach(i = 1:length(multiphylo)) %dopar% {
    output <- corHMM(phy = multiphylo[[i]], data = trait_data, rate.cat = 3, model = "ARD")
    
    # Save output to a file
    saveRDS(output, file = paste0("corHMM_ASR_Caste_only_3rat_ARD_", i, ".rda"))
    cat("Saved output to", paste0("corHMM_ASR_Caste_only_3rat_ARD_", i, ".rda"), "\n")
  }
}
