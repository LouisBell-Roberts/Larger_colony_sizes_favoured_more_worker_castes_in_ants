...
corHMM_multiphylo_2rat_ER <- function(multiphylo, trait_data) {
  ...
  for (i in 1:length(multiphylo)) {
    ...
    output_file <- paste0("corHMM_ASR_Caste_MF_2rat_ER_", i, ".rda")
...
#Read in data file
ant_data <- as.data.frame(read_xlsx("/Users/louis.bell-roberts/Documents/DTP_1st_project_rotation/Data/Master_cloud_data/Publication/ant_data.xlsx", col_names = T))
...
ASRdata <- dplyr::select(ASRdata, animal, queen.mating.frequency, CasteBin)
