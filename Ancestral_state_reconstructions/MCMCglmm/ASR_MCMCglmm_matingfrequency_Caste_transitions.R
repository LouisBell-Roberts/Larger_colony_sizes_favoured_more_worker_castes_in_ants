...
###Analysing queen mating frequency and number of worker castes (monomorphic/polymorphic)
#Louis Bell-Roberts
#29/01/2024
...
#Filter data
sxData <- dplyr::filter(ant_data, caste.number >=1, queen.mating.frequency >=1)
sxData <- dplyr::select(sxData, animal, queen.mating.frequency, CasteBin)
...
##########################################################################################
#ANCESTRAL STATE RECONSTRUCTION WITH QUEEN MATING FREQUENCY AND CASTE NUMBER
##########################################################################################
...
#############
#Combine mating frequency Estimates with Transition Dataset
##Match up ancestor and descendant mating frequency predictions based on row names
...
#Match up tips with mating frequency data based on nodecode column
TransDat_neg_rep <- lapply(TransDat_neg_rep, function(x) {
  x <- data.frame(x, continuous.trait = sxData$queen.mating.frequency[match(x$descendants, sxData$nodecode)])
  return(x)
})

#############
#Assign ancestral nodes to the 'animal' column
#When running the model, this will allow us to estimate mating frequency in 'ancestors'
TransDat_neg_rep <- lapply(TransDat_neg_rep, function(df) {
...
#############
#Model to estimate mating frequency values across each transition type
...
foreach(i = 1:400) %dopar% {
  ...
  saveRDS(model1, file = file.path("1st_run", paste0("MF_Caste_1M_100k_1k_", i, "_1stRun.rds")))
  saveRDS(model2, file = file.path("2nd_run", paste0("MF_Caste_1M_100k_1k_", i, "_2ndRun.rds")))
}
