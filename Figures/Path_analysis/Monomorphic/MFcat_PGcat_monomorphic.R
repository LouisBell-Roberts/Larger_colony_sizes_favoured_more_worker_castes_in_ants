...
all_variables_PGbinary <- dplyr::filter(ant_data, caste.number <2, complete.cases(worker.size.variation), complete.cases(queen.mating.frequency.categorical), complete.cases(colony.size), complete.cases(queen.number.categorical))

#Assign a value of 1 to species with obligate polygyny (only 3 species)
all_variables_PGbinary$queen.number.categorical[all_variables_PGbinary$queen.number.categorical == 2] <- 1

#Assign a value of 0 to species with facultative polyandry
all_variables_PGbinary$queen.mating.frequency.categorical[all_variables_PGbinary$queen.mating.frequency.categorical == 1] <- 0

#Assign a value of 1 to species with obligate polyandry
all_variables_PGbinary$queen.mating.frequency.categorical[all_variables_PGbinary$queen.mating.frequency.categorical == 2] <- 1

#Assign queen.number.categorical and queen.mating.frequency.categorical as factors
all_variables_PGbinary$queen.number.categorical <- as.factor(all_variables_PGbinary$queen.number.categorical)
all_variables_PGbinary$queen.mating.frequency.categorical <- as.factor(all_variables_PGbinary$queen.mating.frequency.categorical)
...
### Main analysis ###
##NCuniform_stem
NCuniform_stem_result <- phylo_path(models, data = all_variables_PGbinary, tree = NCuniform_stem_pruned, method = "logistic_MPLE", model = "lambda", lower.bound = 0, upper.bound = 1)
...
ggsave(filename = "/Users/louis.bell-roberts/Documents/Github/Testing_the_size_complexity_hypothesis_in_ants/Figures/Path_analysis/Plots/NC_uniform_stem_monomorphic_MFcat_PGcat.pdf", width = 6.5, height = 6)

##NCuniform_crown
NCuniform_crown_result <- phylo_path(models, data = all_variables_PGbinary, tree = NCuniform_crown_pruned, method = "logistic_MPLE", model = "lambda", lower.bound = 0, upper.bound = 1)
...
##FBD_stem
FBD_stem_result <- phylo_path(models, data = all_variables_PGbinary, tree = FBD_stem_pruned, method = "logistic_MPLE", model = "lambda", lower.bound = 0, upper.bound = 1)
...
##FBD_crown
FBD_crown_result <- phylo_path(models, data = all_variables_PGbinary, tree = FBD_crown_pruned, method = "logistic_MPLE", model = "lambda", lower.bound = 0, upper.bound = 1)
